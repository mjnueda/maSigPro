...
\author{Ana Conesa and Maria J. Nueda}
\date{30 July 2013}
\maketitle
...
{\bf maSigPro} is a R package that initially was developed for the analysis of single and multiseries time course microarray
experiments. Recently {\bf maSigPro} has been adapted to deal also with Next Generation-Sequencing (NGS) series of data in a proper way.
The usage of this opcion is explained in \ref{section-NGS} section.
...
The on-line help of {\bf maSigPro} can be started by typing at the R prompt:
...
The analysis approach implemented in \textbf{maSigPro} is executed in 5 major steps (Figure \ref{fig:pipeline}) which are run
...
\caption{maSigPro pipeline. Major functions.}
\protect\label{fig:pipeline}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[height=7cm,width=7cm,angle=0]{venn1}
...
function of Time but another variables, such as a physiological parameter measured at the experimental condition, can be used as well 
as the independent variable.\\
The following example illustrates how to build the {\tt edesign} matrix in such case. In this experiment gene expression was analyzed 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Next Generation-Sequencing series}\label{section-NGS}

\textbf{maSigPro} uses \textbf{lm()} function to fit a linear model where statistics for inference use normal distribution. This is the right treatment when dealing
with normal distributed data or big samples. However, for non-normal small samples, results can not be correct.
...
$$Y \sim NB(\theta), Var(Y)=\mu + \frac{\mu}{\theta}$$

\textbf{maSigPro} has been adapted to take into account non-normal distribution of the data. New arguments have been added in \texttt{p.vector} function to deal with this type of data in a proper way:
\begin{description}
\item
{\tt counts:} a logical indicating whether your data are counts. By default is FALSE for microarray treatment.
\item
{\tt theta:} $\theta$ parameter for negative.binomial family. By default $\theta=10$.
{\item}
{\tt family:} the distribution function to be used in the GLM. It must be specified as a function: gaussian(), poisson(), negative.binomial(theta)... If NULL family will be negative.binomial(theta) when counts=TRUE or gaussian() when counts=FALSE.
\end{description}

The recommended analysis to deal with RNA-Seq data is the GLM with negative.binomial family. $\theta$ must be specified and it can be computed by using available methods as edgeR (\citep{Robinson2010}). The application of maSigPro with several values of $\theta$ to the same datasets did not reveal significant differences in gene selection. 
Taking this into consideration we have put by default $\theta=10$ for being an average value. 
Moreover we give to the user the option of applying whatever exponential family to explore other possibilities.

\texttt{NBdata} is a subset of a bigger normalized dataset with 2 experimental groups, 6 time-points and 3 replicates. 
Simulation has been done by using a negative binomial distribution with $\theta=5$ to illustrate this section. The first 20 genes are simulated with changes among time. \texttt{NBdesign} is the design matrix.
...
These genes can be grouped in 4 clusters \ref{fig:NB1}:
...
\begin{figure}
\begin{center}
\includegraphics[width=5in,height=5in]{PlotNB}
\caption{Cluster Analysis significant genes}
\protect\label{fig:NB1}
\end{center}
\end{figure}
...
Changing this arguments in p.vector is enough. Further functions will take these arguments from a \texttt{p.vector} object.
