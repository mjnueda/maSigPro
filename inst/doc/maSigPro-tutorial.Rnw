...
The statistical distribution for tag counts data as Next Generation-Sequencing data may be Poisson or Binomial. Recent studies suggest an overdispersion
of the data, so the negative binomial can model these distributions in a better way.

Recently, \textbf{maSigPro} has been adapted to take into account non-normal distribution of the data. A new argument named \texttt{family} 
has been added to maSigPro. By default \texttt{family=gaussian()}. 
Other families: binomial, poisson, quasipoisson,... See \texttt{?family}. 

We can also use negative.binomial(theta) from MASS package. This family needs the specification
of the \texttt{theta} parameter.

<<eval=FALSE>>=
NGS.p <- p.vector(data, design, family=negative.binomial(2))
NGS.t <- t.fit(NGS.p)
@ 

Changing family in p.vector is enough. Further functions will take this argument from a \texttt{p.vector} object.
